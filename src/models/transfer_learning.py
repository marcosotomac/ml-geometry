...
class TransferLearningModel:
    ...
    def build_model(base_model_name: str = 'efficientnet_b0',
        ...
        if base_model_name not in TransferLearningModel.AVAILABLE_MODELS:
            raise ValueError(
                f"Model {base_model_name} not available. Choose from: {list(TransferLearningModel.AVAILABLE_MODELS.keys())}")
...
class EnsembleModel:
    ...
    def build_ensemble(models_list: list,
        ...
        if ensemble_method == 'average':
            ...
            combined = layers.Dot(axes=1)(
                [weights, layers.Concatenate()(outputs)])
        else:
