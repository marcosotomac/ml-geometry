...
class ModelEvaluator:
    ...
    def plot_confusion_matrix(self, cm: np.ndarray, save_prefix: str):
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_confusion_matrix.png')
    ...
    def save_classification_report(self, report: Dict, save_prefix: str):
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_classification_report.txt')
    ...
    def plot_roc_curves(self, y_true: np.ndarray, y_pred: np.ndarray, save_prefix: str):
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_roc_curves.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_precision_recall_curves(self, y_true: np.ndarray, y_pred: np.ndarray, 
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_pr_curves.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_per_class_accuracy(self, per_class_acc: List[float], save_prefix: str):
        ...
        bars = plt.bar(range(self.num_classes), per_class_acc, color='skyblue', edgecolor='navy')
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_per_class_accuracy.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def visualize_predictions(self, test_generator, save_prefix: str, 
        ...
        save_path = os.path.join(self.output_dir, f'{save_prefix}_sample_predictions.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_training_history(history: Dict, output_dir: str = 'results', 
    ...
    if 'precision' in history:
        ax.plot(history.get('precision', []), label='Train Precision', linewidth=2)
        ax.plot(history.get('val_precision', []), label='Val Precision', linewidth=2)
